...
std::size_t GroceryList::size() const
{
  ...
  return GroceryList::gList_sll_size();
...
void GroceryList::insert( const GroceryItem & groceryItem, Position position )
{
  ...
  if( position == Position::TOP ) insert( groceryItem, 0 );
    ...
    insert( groceryItem, size() );
  else
    throw std::logic_error( "Unexpected insertion position" exception_location );    // Programmer error.  Should never hit this!
}
...
void GroceryList::insert( const GroceryItem & groceryItem, std::size_t offsetFromTop )    // insert provided grocery item at offsetFromTop, which places it before the current grocery item at offsetFromTop
{
  ...
  {
    throw InvalidOffset_Ex( "Insetion is a duplication returning size" exception_location );
  }
...

  {
    ...
    {
      std::move_backward( Position::TOP, Position::BOTTOM, Position::BOTTOM ); //*
      ++_gList_array_size;
    }
    else
    {
      throw CapacityExceeded_Ex( "_gList_array_size is not less than _gList_array.size()" exception_location );
    }
...

  {
      ...
      std::next( _gList_vector.begin(), offsetFromTop);
...

  {
    ...
    std::next( _gList_dll.begin(), offsetFromTop );
  ...
  { /**********  Part 4 - Insert into singly linked list  **********/
    ...
    std::next( _gList_sll.begin(), offsetFromTop );
...
void GroceryList::remove( std::size_t offsetFromTop )
{
...

  {
      ...
      std::move(Position::TOP, Position::BOTTOM, //*
                Position::TOP );
